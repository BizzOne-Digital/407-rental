...
export function SaveBar({ onSave, saving = false, label = 'Save Changes' }: SaveBarProps) {
  ...
  return (
    <div className="sticky bottom-0 z-10 -mx-6 -mb-6 mt-8 flex items-center justify-between border-t border-brand-grey/10 bg-brand-white px-6 py-4 shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
      <div>
        {status === 'success' && (
          <span className="text-sm font-semibold text-brand-red">✓ Saved successfully — live on website now</span>
        )}
        {status === 'error' && (
          <span className="text-sm font-semibold text-brand-red">{errorMsg}</span>
        )}
      </div>
      <button
        type="button"
        onClick={handleSave}
        disabled={saving}
        className="rounded-sm bg-brand-red px-8 py-3 text-sm font-bold text-brand-white transition-colors hover:bg-brand-red/90 disabled:opacity-50"
      >
        {saving ? 'Saving...' : label}
      </button>
    </div>
...
export function TextInput({
  ...
  return (
    <input
      ...
      className="w-full rounded-sm border border-brand-grey/20 px-4 py-2.5 text-sm focus:border-brand-red focus:outline-none"
    />
  )
}
